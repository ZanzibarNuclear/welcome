...
useHead({
  ...
  meta: [{ name: "description", content: simulator.value.description }],
});

// Import the simulator component dynamically if specified
const simulatorComponentName = simulator.value.component;
const simulatorComponentLoaders = import.meta.glob<{ default: Component }>(
  "../../components/simulators/*.vue",
);
...
const SimulatorComponent = simulatorLoader
  ? defineAsyncComponent(() =>
      simulatorLoader().then((module) => module.default),
    )
  : null;
</script>
      ...
      <div class="flex flex-wrap items-center gap-3 mb-4">
        <span
          :class="[
            ...
            simulator.difficulty === 'beginner'
              ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300'
              : simulator.difficulty === 'intermediate'
