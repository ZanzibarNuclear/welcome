...
It does not simulate **3D bends**, **water hammer**, **free surfaces**, **heat transfer** along the pipe, or **exact** tabulated \(K\) catalogs (you choose \(\sum K\) yourself). Temperature affects fluid properties only through the built-in \(\rho(T)\), \(\mu(T)\) tables.
...
- Add **ΣK** and compare **pipe** vs **minor** head; check the **equivalent length** hint.
